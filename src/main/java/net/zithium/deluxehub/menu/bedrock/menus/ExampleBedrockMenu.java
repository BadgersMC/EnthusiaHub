        ...
        public Form getForm() {
            return SimpleForm.builder()
                    ...
                    .validResultHandler(response -> {
                        ...
                        switch (buttonId) {
                            case 0 -> {
                                // Server Selector - Open another menu
                                player.sendMessage("§aOpening server selector...");
                                // bedrockNavigator.openMenu(new ServerSelectorMenu(...));
                            }
                            case 1 -> {
                                // Player Settings
                                player.sendMessage("§bOpening player settings...");
                                // bedrockNavigator.openMenu(new PlayerSettingsMenu(...));
                            }
                            case 2 -> {
                                // Cosmetics
                                player.sendMessage("§eOpening cosmetics...");
                                // bedrockNavigator.openMenu(new CosmeticsMenu(...));
                            }
                            case 3 -> {
                                // Close
